...
const bot = new Eris('put your bot token here', {
    intents: 32509,
    allowedMentions: {
        everyone: true,
        roles: true
...
bot.on("ready", async () => {
    ...
    await bot.editStatus("online", { type: 0, name: process.env["status"] || "By Subtreem" })
    ...
    if (interaction.type == 5) {
        if (interaction.data.custom_id.startsWith("ct")) {
            ...
            await interaction.createMessage({
                embeds: [{
                    description: `☑️ The ticket has been locked by <@!${interaction.member.id}>`,
                    fields: [{ name: "Reason", value: `\`\`\`${interaction.data.components[0].components[0].value}\`\`\`` }],
                    color: 0xdfb21f
                }]
            }).catch(() => { });
            ...
            setTimeout(async () => {
                await bot.createMessage(interaction.data.custom_id.split("_")[4], {
                    embeds: [{
                        title: "The ticket has been locked",
                        description: `The ticket has been locked by <@!${interaction.member.id}>`,
                        fields: [{ name: "Ticket", value: `<#${interaction.data.custom_id.split("_")[2]}>` }, { name: "Reason", value: `\`\`\`${interaction.data.components[0].components[0].value}\`\`\`` }, { name: "Ticket type", value: `\`\`\`${interaction.data.custom_id.split("_")[3].replaceAll("-", " ")}\`\`\`` }, { name: "Ticket by", value: `<@!${interaction.data.custom_id.split("_")[1]}>` }],
                        color: 0xdfb21f,
                        timestamp: new Date()
                    }]
                }, {
    ...
    if (interaction.type == 3) {
        if (interaction.data.custom_id.startsWith("ct_")) {
            ...
            if (interaction.data.custom_id.split("_")[1] != interaction.member.id && !interaction.member.roles.some(x => x == interaction.data.custom_id.split("_")[5])) return await interaction.createMessage({
                flags: 64,
                content: "Only the ticket owner or support staff can lock the ticket from this button :x:"
            })
            await bot.createInteractionResponse(interaction.id, interaction.token, {
                type: 9, data: {
                    "title": "Find out why the ticket was locked",
                    ...
                    "components": [
                        {
                            ...
                            "components": [
                                {
                                    "type": 4,
                                    "label": "Write the reason for locking the ticket",
                                    "style": 2,
                                    "custom_id": "reason",
        ...
        if (interaction.data.custom_id.startsWith("tt_")) {
            ...
            if (!interaction.member.roles.some(x => x == interaction.data.custom_id.split("_")[5])) return await interaction.createMessage({
                flags: 64,
                content: "Only technical support staff can perform this procedure :x:" + `\n<@&${interaction.data.custom_id.split("_")[5]}>`
            })
            await interaction.defer()
            let msgs = await bot.getMessages(interaction.channel.id, { limit: 500 })
            await interaction.createFollowup({
                embeds: [{
                    title: "The ticket has been transferred to the chat",
                    description: `The ticket was forwarded to the conversation by <@!${interaction.member.id}>`,
                    color: 0xdfb21f,
                    timestamp: new Date()
                }]
            }, {
        ...
        if (interaction.data.custom_id.startsWith("dt_")) {
            ...
            await interaction.createMessage({ flags: 64, content: "Your request is being processed" })
            let msgs = await bot.getMessages(interaction.channel.id, { limit: 500 })
            setTimeout(async () => {
                await bot.deleteChannel(interaction.data.custom_id.split("_")[2])
                await bot.createMessage(interaction.data.custom_id.split("_")[4], {
                    embeds: [{
                        title: "The ticket has been deleted",
                        description: `Ticket deleted by <@!${interaction.member.id}>`,
                        fields: [{ name: "Ticket", value: `${interaction.data.custom_id.split("_")[2]}` }, { name: "Ticket type", value: `\`\`\`${interaction.data.custom_id.split("_")[3].replaceAll("-", " ")}\`\`\`` }, { name: "Ticket by", value: `<@!${interaction.data.custom_id.split("_")[1]}>` }],
                        color: 0xdf1f1f,
                        timestamp: new Date()
                    }]
                }, {
        ...
        if (interaction.data.custom_id.startsWith("ot")) {
            ...
            await bot.createMessage(interaction.data.custom_id.split("_")[4], {
                embeds: [{
                    title: "The ticket has been opened",
        ...
        if (interaction.data.custom_id.startsWith("ticket")) {
            ...
            bot.createChannel(interaction.guildID, `ticket-${padNum(count + 1, 4)}`, 0, {
                ...
                await interaction.editOriginalMessage({
                    flags: 64,
                    content: `☑️ Your ticket has been created <#${channel.id}>`,
                })
                await bot.createMessage(interaction.data.custom_id.split("_")[4], {
                    embeds: [{
                        title: "The ticket has been created",
                        description: `Ticket created by <@!${interaction.member.id}>`,
                        fields: [{ name: "Ticket", value: `<#${channel.id}>` }, { name: "Ticket type", value: `\`\`\`${interaction.data.custom_id.split("_")[1].replaceAll("-", " ")}\`\`\`` }, { name: "Reason", value: `\`\`\`${interaction.data.custom_id.split("_")[5].replaceAll("-", " ")}\`\`\`` }],
                        color: 0x3FB745,
                        timestamp: new Date()
                    }]
                })
                    ...
                    content: `Welcome , <@!${interaction.member.id}>, <@&${interaction.data.custom_id.split("_")[2]}>`, embeds: [
                        {
                            ...
                            description: `To create the ticket. Please wait for the support team. They will respond to you. If you want to lock the ticket ، press the button 🔒${interaction.data.custom_id.split("_")[5] == true ? "" : `\n\`\`\`${interaction.data.custom_id.split("_")[5].replaceAll("-", " ")}\`\`\` `}`,
                            color: 0x3FB745,
                            footer: { text: "Please follow the Rules" }
                        }
                    ],
                    components: [{
                        type: 1, components: [{
    ...
    if (interaction.type == 2) {
        if (interaction.data.name == "setup") {
            if (!interaction.member.permissions.has("administrator")) return await interaction.createMessage({
                flags: 64,
                content: ":x: You do not have permissions for this"
            })
            ...
            await interaction.createMessage({
                flags: 64,
                embeds: [{
                    title: "☑️ Your request has been executed successfully",
                    color: 0x3FB745
                }]
            });
            let components = [];
